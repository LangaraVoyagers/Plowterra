...
const cardStruc = (
  ...
  const CIRCULAR_LOADER = <CircularProgress color="secondary" size="1.75rem" />;
  ...
  return {
    harvestInfo: [
      ...
      },
      {
        ...
        content: isLoading ? CIRCULAR_LOADER : <FormattedDate value={season?.startDate} year="numeric" month="short" day="2-digit" />
      },
      {
        label: <FormattedMessage defaultMessage="PRICE PER UNIT" id="dashboard.labels.pricePerUnit" />,
        content: isLoading ? CIRCULAR_LOADER : `${ currencyName } ${ Number(season?.price).toFixed(2) }`
      },
      {
      ...
      },
      {
        ...
        content: isLoading ? CIRCULAR_LOADER : `${ Number(totals?.totalHarvest).toFixed(2) } ${ unitName }`
      }
    ],
    midSectionInfo: [
      {
        label: <FormattedMessage defaultMessage="SEASON'S TOTAL PAYROLL" id="dashboard.labels.seasonsTotalPayroll" />,
        content: isLoading ? CIRCULAR_LOADER : `${ currencyName } ${ Number(totals?.totalPayroll).toFixed(2) }`,
        perIncrease: isLoading ? `- %` : `${ averages?.avePayrollChange?.toFixed(2) ?? 0 } %`
      },
      {
        label: <FormattedMessage defaultMessage="TODAY'S COLLECTION" id="dashboard.labels.todaysCollection" />,
        content: isLoading ? CIRCULAR_LOADER : `${ currencyName } ${ Number(payrollToTodayData?.grossAmount).toFixed(2) }`,
        perIncrease: isLoading ? `- %` : `${ averages?.aveHarvestChange?.toFixed(2) ?? 0 } %`
      }
    ],
    payrollInfo: {
      payrollToToday: {
        label: <FormattedMessage defaultMessage="PAYROLL TO THIS DAY" id="dashboard.labels.payrollToThisDate" />,
        content: isLoading ? CIRCULAR_LOADER : `${ currencyName } ${ Number(payrollToTodayData?.grossAmount).toFixed(2) }`,
        startDate: <FormattedDate value={payrollToTodayData?.startDate} month="short" day="2-digit" />,
        daysLeft: isLoading ? '-' : (payrollToTodayData?.daysRemaining > 0 ? payrollToTodayData?.daysRemaining : 0)
      },
